...
import "./SdlBaseRewardPool.sol";
import "./SdlVirtualBalanceRewardPool.sol";
...
contract RewardFactory {
    ...
    //Create a Managed Reward Pool to handle distribution of all sdl mined in a pool
    function CreateSdlRewards(uint256 _pid, address _depositToken)
        external
        returns (address)
    {
        require(msg.sender == operator, "!auth");

        //operator = booster(deposit) contract so that new sdl can be added and distributed
        //reward manager = this factory so that extra incentive tokens(ex. snx) can be linked to the main managed reward pool
        SdlBaseRewardPool rewardPool = new SdlBaseRewardPool(
            _pid,
            _depositToken,
            sdl,
            operator,
    ...
    //used for extra incentive tokens(ex. snx) as well as vesdl fees
    function CreateTokenRewards(
        ...
        SdlVirtualBalanceRewardPool rewardPool = new SdlVirtualBalanceRewardPool(
                _mainRewards,
                _token,
                _operator
            );
    ...
    /**
     * @dev Add an extra reward.
     */
    function addExtraReward(uint256 _pid, address _extraReward) external {
        require(IDeposit(operator).owner() == msg.sender, "!owner");

        (, , , address mainRewardContract, , ) = IDeposit(operator).poolInfo(
            _pid
        );
        IRewards(mainRewardContract).addExtraReward(_extraReward);
    }
}
